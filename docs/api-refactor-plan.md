...
状态：重构实施中。审查基线：`a4db9fbe1c1480bcbaa3bab203d1bd9e37556621`，2026-09-24；当前工作副本更新于 2026-09-26。THYou App 的 pubspec 已固定到 public alpha commit `6275bbeb` 并通过消费者静态检查，但生产 gateway 仍使用 App 内旧 Runtime。未发布的 `0.2.0-alpha.1` Flutter facade 现已覆盖双账号 Auth、本机网络资料、Identity cookie 快照的显式恢复与重新验证、网上服务大厅、SelfService、Registrar/Calendar、INFO、Learn、Library、Classroom、CampusCard 和 Electricity 的首批 API；这些 facade 使用同一个 Rust Client，但 App 自身仍未迁移其 gateway、完整账号 hydration 或页面数据流。Identity 快照是加密目录实现，密钥与密文同处；它不是 OS Keychain，SelfService 持久状态也未恢复。Rust 默认 User-Agent 与受信设备展示名使用 `TsinghuaKit`；旧 THYou 容器、缓存和兼容文件名仍保留，避免对既有安装造成隐式存储迁移。App 的 72 个旧 gateway 方法逐项映射见 [Flutter API 迁移矩阵](flutter-api-migration-matrix.md)。
...
校园网领域契约已明确区分 `Portal` 与 `SystemWifiEap` 两种本机资料用途。Rust facade 已能在 owning Client 生命周期内保存资料及显式选择保存在内存中的密码，并能准备不含密码的表单数据；当前默认不持久化；Rust 的 `NetworkProfileStoragePolicy::EncryptedDirectory` 可跨 Client/进程恢复，但 key 与密文同处目录；Flutter App 应选择 `NetworkProfilePersistence.platformSecureStorage`，由 Flutter Secure Storage 保管 32-byte 加密密钥、Rust 保管加密 Profile 文件。切换到安全存储策略时 engine 会在锁内解密旧目录数据、用新 key 原子重写并移除旧同目录 key。密码仍需通过匹配当前版本的 `PreparedNetworkInput` 显式取得为脱敏、非 `Clone` 的 `NetworkProfilePassword`。真实连接入口仍不支持 Portal 连接执行或操作系统 802.1X 配置；`SystemWifiEap` 不代表当前 App 已能自动填入 Tsinghua Secure 系统设置。系统连接必须按平台能力和 OS 授权返回明确结果。

仍未完成的主要工作：更多 THOS/能力目录、Identity/SelfService 凭据安全存储和恢复、Flutter App 生产迁移、Portal 连接和系统网络适配入口尚未逐一迁入 Client；网络 Profile 的平台 secure-storage key 路径已接入但尚未在真实 Keychain/Keystore 上做运行验证。当前 Learn 作业详情是只读文本和附件元数据，不提供提交、下载或预览；讨论只公开列表，不公开帖子详情或发帖；课程资料保存不提供进程内预览，也不会覆盖已有路径。SelfService 目前提供账号摘要、在线设备查询、用量/余额及基于最新列表 `DeviceRef` 的显式断开，但细粒度缓存与按用途分离的持久凭据策略未迁移；OS Keychain/宿主 secret-store adapter、Flutter/FRB 密码填充通道、Portal 连接执行、Tsinghua Secure 系统适配器、FRB/Flutter 包独立生成与 App 消费切换也未完成。`SystemWifiEap` 目前只是网络资料领域枚举，不能代表已能写入操作系统配置。现有 FFI 包仍保留旧 `CampusRuntime` 兼容入口。校园卡 facade 仍使用 legacy Runtime `String` 失败，所以除当前 Identity/交互状态外其他错误只归为稳定 `ServiceUnavailable`；尚未进行真实账号访问。
