...
状态：重构实施中。审查基线：`a4db9fbe1c1480bcbaa3bab203d1bd9e37556621`，2026-09-24；当前工作副本更新于 2026-09-26。THYou App 工作树现已固定 public SDK commit `5ba172a`，依赖解析与 macOS Debug 构建通过；但 `lib/` 仍没有 `package:tsinghua_kit` 生产导入，provider 与数据流继续使用旧 Runtime。工作树中的 `0.2.0-alpha.1` Flutter facade 已覆盖双账号 Auth、Identity 快照恢复、本机网络资料和多项校园服务；Identity 快照现可选择由宿主安全存储持有的独立 32-byte key，旧同目录 key 策略仅供兼容。真实设备 Keychain/Keystore 尚未验证，Auth 密码保存现通过独立的 CredentialStoragePolicy/AuthCredentialPersistence 配置，与 Identity 会话快照、业务缓存和网络 Profile 分离；Identity 与 SelfService 使用独立命名空间，各登录的 rememberCredentials 默认关闭且逐次选择。密码只在认证成功后保存。Identity 跨进程恢复仍要求单独开启 Identity 快照并显式重新验证；SelfService saved login 每次仍需人工完成图片验证码。凭据密钥目前保存在 app-private 目录，不属于 OS Keychain。Rust 与 Flutter 必须保持 Identity、SelfService 两个 Auth 域；Portal/Tsinghua Secure 是本机网络输入/操作，不是第三个账号。App 的 72 个旧 gateway 方法逐项映射见 [Flutter API 迁移矩阵](flutter-api-migration-matrix.md)。
...
校园网领域契约已明确区分 `Portal` 与 `SystemWifiEap` 两种本机资料用途。Rust facade 可在 owning Client 生命周期内保存资料及用户显式选择保存在内存中的密码、准备不含密码的表单数据；还可通过同一 Client 生成的当前版本 `PreparedNetworkInput` 明确发起 Portal 连接，并仅断开当前进程内经验证的一次性目标。密码覆盖只用于这一次 Portal 操作，不持久化；省略覆盖值时 Rust 只读取资料中用户已显式保存的密码。`SystemWifiEap` 在任何 Portal 请求之前被拒绝。当前默认不持久化；Rust 的 `NetworkProfileStoragePolicy::EncryptedDirectory` 可跨 Client/进程恢复，但 key 与密文同处目录；Flutter App 应选择 `NetworkProfilePersistence.platformSecureStorage`，由 Flutter Secure Storage 保管 32-byte 加密密钥、Rust 保管加密 Profile 文件。切换到安全存储策略时 engine 会在锁内解密旧目录数据、用新 key 原子重写并移除旧同目录 key。显式表单填入仍需通过匹配当前版本的 `PreparedNetworkInput` 取得脱敏、非 `Clone` 的 `NetworkProfilePassword`。Tsinghua Secure 的操作系统 802.1X 配置仍未实现，必须按平台能力和 OS 授权返回明确结果；资料保存/填入不能显示为已连接。

仍未完成的主要工作：更多 THOS/能力目录、Auth 凭据 vault 的 OS 安全存储、SelfService 跨进程会话恢复、Flutter App 生产迁移和操作系统 Wi-Fi/EAP 适配入口；网络 Profile 与 Identity 快照的平台 secure-storage key 路径已接入但尚未在真实 Keychain/Keystore 上做运行验证。当前 Learn 作业详情是只读文本和附件元数据，不提供提交、下载或预览；讨论只公开列表，不公开帖子详情或发帖；课程资料保存不提供进程内预览，也不会覆盖已有路径。SelfService 目前提供账号摘要、在线设备查询、用量/余额及基于最新列表 `DeviceRef` 的显式断开，也可在明确选择保存后通过 Rust 启动新的图片验证码流程；它不恢复会话或绕过验证码。仍未实现 OS Keychain/宿主 secret-store adapter、Tsinghua Secure 系统适配器、FRB/Flutter 包独立发布与 App 消费切换。`SystemWifiEap` 不能代表已能写入操作系统配置。现有 FFI 包仍保留旧 `CampusRuntime` 兼容入口。校园卡 facade 仍使用 legacy Runtime `String` 失败，所以除当前 Identity/交互状态外其他错误只归为稳定 `ServiceUnavailable`；尚未进行真实账号访问。
...
存储至少区分：按认证域/账号隔离的业务缓存、受控会话快照、信任设备元数据、两个认证域各自的可选凭据、本机 Portal/EAP 连接资料及其各自可选密码。即使四种用途使用相同 username，也使用不同存储键和加密用途标识。OS 原生 Wi-Fi 凭据不由 SDK 资料记录代替；更改应用资料不能隐式覆写 OS 配置。不能把上述用途合并为一个可随意导出的 `ClientState` JSON。默认使用内存，宿主选择持久化后仍须遵守各项 opt-in。
...
| P4：交互与网络边界（进行中） | 统一身份/自助交互、域级注销恢复；完成 Client 内 Portal/EAP 资料、显式密码填充和 opt-in Unix 加密目录持久化，之后实现 平台 secure-storage key 适配、Portal connector、按平台能力适配系统 Wi-Fi；连接观测/操作从 Auth 移出；迁移本机资料旧 key | 串行认证、禁止不明确重放、用途/账号隔离、尊重 OS 授权 | 保存/填写不发连接请求；EAP 不走 srun；网络连接不生成账号会话；平台能力、账号变化和依赖失效均可验证 |
...
## 45. 2026-09-26 Identity 快照宿主安全密钥
