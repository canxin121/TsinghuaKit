...
状态：重构实施中。自 2026-09-26 起，凭据、Identity 会话快照和 NetworkProfile 的持久化均由 TsinghuaKit 管理应用私有目录内的加密文件；密钥与各自密文同目录，不使用 Keychain、Keystore、Flutter Secure Storage 或其它系统凭据存储。逐次记住密码默认关闭，Identity 与 SelfService 账号域分开保存。该文件模式不防范可读取应用私有目录的同一用户进程。THYou App 仍在迁移旧 Runtime，具体状态以本节的 SDK/App 集成记录为准；历史段落中提到的系统安全存储模式已被后续决定废弃。Rust 与 Flutter 必须保持 Identity、SelfService 两个 Auth 域；Portal/Tsinghua Secure 是本机网络输入/操作，不是第三个账号。App 的 72 个旧 gateway 方法逐项映射见 [Flutter API 迁移矩阵](flutter-api-migration-matrix.md)。
...
## 50. 2026-09-26 统一采用 SDK 管理的文件持久化

后续决定是不依赖系统凭据存储。Flutter Secure Storage 和 HostSecureStorage / KeychainEncryptedDirectory API 已从 Rust SDK、FFI 与 Dart facade 移除，`flutter_secure_storage` 依赖也已删除。显式保存的 Identity 与 SelfService 凭据、Identity 会话快照和 NetworkProfile 分别写入应用私有目录中的加密文件；TsinghuaKit 在相应目录管理文件密钥和锁，不把它交给 Flutter，也不把密钥与不同用途的数据混用。所有保存仍需用户逐次 opt-in，默认只保存在 Client 生命周期内。此方案方便由应用管理文件与备份，但不能保护数据免受可读该应用目录的同一用户进程或管理员访问。

统一身份与网络自助仍是仅有的两个 Auth 域。校园网 Portal/Tsinghua Secure 账号可作为本机网络资料保存和显式填写，不参与 Auth 状态、会话快照或恢复链。失效或格式错误的文件返回存储错误并保留原始文件，不静默旋转密钥、清空资料或伪装成空结果。旧 HostSecureStorage 加密 envelope 不自动迁移；在采用过期宿主密钥格式的应用中，需先按旧版本导出或清理对应数据，再切换至新的 SDK 文件格式。

验证：本轮移除宿主密钥的 FFI / Dart API，修复公开入口测试并为凭据目录结构增加定向断言；FRB 源码由 codegen 重新生成。定向 Rust、Flutter 检查与测试结果见本轮交付记录。未执行真实账号登录或任何学校服务请求。
