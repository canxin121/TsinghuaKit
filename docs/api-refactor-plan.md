...
状态：重构实施中。审查基线：`a4db9fbe1c1480bcbaa3bab203d1bd9e37556621`，2026-09-24；当前工作副本更新于 2026-09-26。THYou App 仍锁定 `v0.1.1` 兼容入口。未发布的 `0.2.0-alpha.1` Flutter facade 现已覆盖双账号 Auth、本机网络资料、Identity cookie 快照的显式恢复与重新验证、网上服务大厅、SelfService、Registrar/Calendar、INFO、Learn、Library、Classroom、CampusCard 和 Electricity 的首批 API；这些 facade 使用同一个 Rust Client，但 App 自身仍未迁移其 gateway、完整账号 hydration 或页面数据流。Identity 快照是加密目录实现，密钥与密文同处；它不是 OS Keychain，SelfService 持久状态也未恢复。Rust 默认 User-Agent 与受信设备展示名使用 `TsinghuaKit`；旧 THYou 容器、缓存和兼容文件名仍保留，避免对既有安装造成隐式存储迁移。App 的 72 个旧 gateway 方法逐项映射见 [Flutter API 迁移矩阵](flutter-api-migration-matrix.md)。
...
校园网领域契约已明确区分 `Portal` 与 `SystemWifiEap` 两种本机资料用途。Rust facade 已能在 owning Client 生命周期内保存资料及显式选择保存在内存中的密码，并能准备不含密码的表单数据；当前默认不持久化；调用者显式配置 `NetworkProfileStoragePolicy::EncryptedDirectory` 后，资料可跨 Client/进程恢复。该 Unix 文件后端使用 ChaCha20-Poly1305、宿主命名空间、原子替换、私有文件权限和进程独占锁；key 与密文同处目录，因此不是 OS Keychain，也不防同一 OS 用户或管理员。密码仍需通过匹配当前版本的 `PreparedNetworkInput` 显式取得为脱敏、非 `Clone` 的 `NetworkProfilePassword`。真实连接入口仍不支持 Portal 连接执行或操作系统 802.1X 配置；`SystemWifiEap` 不代表当前 App 已能自动填入 Tsinghua Secure 系统设置。系统连接必须按平台能力和 OS 授权返回明确结果。

仍未完成的主要工作：更多 THOS/能力目录以及 OS Keychain/宿主 secret-store adapter、Flutter/FRB 密码填充接入、Portal 连接和系统网络适配入口尚未逐一迁入 Client；受限的 Unix 加密目录后端已实现但需显式选择。当前 Learn 作业详情是只读文本和附件元数据，不提供提交、下载或预览；讨论只公开列表，不公开帖子详情或发帖；课程资料保存不提供进程内预览，也不会覆盖已有路径。SelfService 目前提供账号摘要、在线设备查询、用量/余额及基于最新列表 `DeviceRef` 的显式断开，但细粒度缓存与按用途分离的持久凭据策略未迁移；OS Keychain/宿主 secret-store adapter、Flutter/FRB 密码填充通道、Portal 连接执行、Tsinghua Secure 系统适配器、FRB/Flutter 包独立生成与 App 消费切换也未完成。`SystemWifiEap` 目前只是网络资料领域枚举，不能代表已能写入操作系统配置。现有 FFI 包仍保留旧 `CampusRuntime` 兼容入口。校园卡 facade 仍使用 legacy Runtime `String` 失败，所以除当前 Identity/交互状态外其他错误只归为稳定 `ServiceUnavailable`；尚未进行真实账号访问。
...
| P4：交互与网络边界（进行中） | 统一身份/自助交互、域级注销恢复；完成 Client 内 Portal/EAP 资料、显式密码填充和 opt-in Unix 加密目录持久化，之后实现 OS Keychain adapter、Portal connector、按平台能力适配系统 Wi-Fi；连接观测/操作从 Auth 移出；处理旧存储迁移 | 串行认证、禁止不明确重放、用途/账号隔离、尊重 OS 授权 | 保存/填写不发连接请求；EAP 不走 srun；网络连接不生成账号会话；平台能力、账号变化和依赖失效均可验证 |
