...
async fn backend_repair_deep_api_tunet_malformed_account_identifier_cannot_prove_online() {
    ...
    assert!(r.load_tunet_status().await.is_err());
    assert!(!r.service_session_is_proven(ServiceId::Tunet));
...
fn tunet_runtime(server: &FixtureServer) -> CampusRuntime {
    let mut r = runtime();
    prove(&mut r, ServiceId::Tunet, "network-b");
    r.tunet_username = Some("network-b".into());
    r.tunet_ip = Some("192.0.2.10".into());
...
async fn backend_repair_deep_api_tunet_matching_username_and_ip_are_usable() {
    ...
    let mut r = tunet_runtime(&s);
    assert!(r.load_tunet_status().await.unwrap().online);
    assert_eq!(s.requests().len(), 1);
}
...
async fn backend_repair_tunet_physical_ip_probe_does_not_create_logout_authority() {
    for (body, expected_state) in [
        ...
        let client = runtime.tunet_client.take().unwrap();
        runtime.coordinator.logout(ServiceId::Tunet).unwrap();
        runtime.tunet_username = None;
        runtime.tunet_ip = None;
        let status = runtime
            .load_tunet_status_using(client, "192.0.2.10".into(), false)
            .await
            .unwrap();
