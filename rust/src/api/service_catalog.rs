...
pub(crate) fn enable_automatic_readers(
    ...
    for entry in &mut catalog.services {
        ...
        let snapshot = snapshot_for_catalog_service(coordinator, service);
...
fn capability_is_proven(
    ...
    match contract.proof {
        "tunet_local_status" | "tunet_login_action" => true,
        ...
        "tunet_service_session" => {
            independent_service_session_is_current(coordinator, CatalogServiceId::Tunet)
                && proofs.tunet
        }
        _ => false,
    }
...
fn protocol_service_id(service: CatalogServiceId) -> ServiceId {
    match service {
        CatalogServiceId::Identity => ServiceId::Identity,
        CatalogServiceId::Learn => ServiceId::Learn,
        CatalogServiceId::Registrar => ServiceId::Registrar,
        CatalogServiceId::Info => ServiceId::Info,
        CatalogServiceId::Usereg => ServiceId::Usereg,
        CatalogServiceId::Tunet => ServiceId::Tunet,
        CatalogServiceId::Library => ServiceId::Library,
        CatalogServiceId::CampusCard => ServiceId::CampusCard,
        ...
        CatalogServiceId::Classroom | CatalogServiceId::Electricity => ServiceId::Info,
    }
}
...
fn snapshot_for_catalog_service(
    ...
    coordinator
        .registry()
        .snapshot_for(protocol_service_id(service))
}
...
fn shared_service_session_is_current(
    ...
    let snapshot = snapshot_for_catalog_service(coordinator, service);
    service_session_is_current(&snapshot, identity)
}
...
fn independent_service_session_is_current(
    ...
    is_currently_authenticated(&snapshot_for_catalog_service(coordinator, service))
}
...
mod tests {
    ...
    #[test]
    fn backend_repair_tunet_status_and_login_are_public_but_logout_needs_srun_proof() {
