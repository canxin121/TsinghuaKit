...
async fn backend_repair_api_audit_tunet_other_device_status_preserves_but_does_not_prove_ours() {
    ...
    assert!(runtime.load_tunet_status().await.is_err());
...
fn tunet_runtime(server: &FixtureServer) -> CampusRuntime {
    ...
    runtime.tunet_username = Some(user().username);
    runtime.tunet_ip = Some("192.0.2.10".into());
    prove(&mut runtime, ServiceId::Tunet);
    runtime
}
...
async fn backend_repair_api_audit_tunet_unknown_status_does_not_claim_logout() {
    ...
    let mut runtime = tunet_runtime(&server);
    assert!(
        runtime.load_tunet_status().await.is_err(),
        "not an online proof"
    );
    assert!(
        runtime.service_session_is_proven(ServiceId::Tunet),
...
#[tokio::test]
async fn backend_repair_api_audit_tunet_proven_offline_clears_network_session() {
