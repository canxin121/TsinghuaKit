...
pub enum ServiceAuthenticationSource {
    ...
    ServiceSession,
    IndependentLogin,
    /// Local portal or operating-system network state, not an Auth session.
    LocalNetworkContext,
    AdapterUnavailable,
}
...
impl ServiceAuthenticationSource {
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::LocalNetworkContext => "local_network_context",
...
pub enum AuthenticationRequirement {
    ...
    /// A local network context or process-local connection target is needed.
    LocalNetworkContext,
}
...
impl ServiceCapability {
    ...
    pub const fn contract_for(
        ...
        let (proof, route) = match (service, self) {
            ...
            (Tunet, Self::Refresh) => ("tunet_local_status", "refresh"),
            (Tunet, Self::ReadNetworkStatus) => ("tunet_local_status", "tunet_status"),
            (Tunet, Self::ConnectNetwork) => ("tunet_connect_action", "tunet_connect"),
            (Tunet, Self::DisconnectNetwork) => ("tunet_connection_target", "tunet_disconnect"),
    ...
    /// boundaries. Learning/INFO/library/card refresh requires both the
    /// unified identity session and the target service proof. TUNet observes
    /// local network state, while USEREG uses its separate Auth account.
    ...
    pub const fn metadata_for(self, service: CatalogServiceId) -> ServiceCapabilityMetadata {
        ...
        let authentication = match (self, service) {
            (Self::Refresh, CatalogServiceId::Tunet)
            | (Self::DisconnectNetwork, CatalogServiceId::Tunet) => {
                AuthenticationRequirement::LocalNetworkContext
            }
            (Self::ReadNetworkStatus | Self::ConnectNetwork, CatalogServiceId::Tunet) => {
...
pub const STANDARD_SERVICE_DEFINITIONS: &[ServiceDefinition] = &[
    ...
    ServiceDefinition {
        ...
        default_availability: ServiceAvailability::Available,
        authentication_source: ServiceAuthenticationSource::LocalNetworkContext,
        visible_by_default: true,
    },
    ServiceDefinition {
