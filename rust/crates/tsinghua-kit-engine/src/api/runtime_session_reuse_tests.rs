...
#[test]
fn backend_refactor_sdk_identity_snapshot_is_frozen_after_first_commit() {
    use reqwest::cookie::CookieStore as _;

    let root = persistence_root("sdk-identity-snapshot-once");
    let _lease = crate::session_persistence::begin_explicit_authority_with_opt_in(
        &root,
        "fixture-user",
        false,
    )
    .expect("fixture Identity authority");
    let mut runtime = persistent_runtime(&root);
    runtime.sdk_identity_snapshot_once = true;
    runtime
        .coordinator
        .begin_authentication(ServiceId::Identity)
        .expect("Identity login boundary begins");
    runtime
        .coordinator
        .mark_authenticated(ServiceId::Identity, user(), None, None, None)
        .expect("fixture Identity proof");

    let identity_url = Url::parse("https://identity.fixture.invalid/").unwrap();
    let self_service_url = Url::parse("https://usereg.fixture.invalid/").unwrap();
    runtime
        .identity
        .transport()
        .cookie_jar()
        .add_cookie_str("identity_session=fixture; Path=/; Secure", &identity_url);
    runtime
        .persist_resume_state(&user())
        .expect("first Identity checkpoint is saved");
    assert!(runtime.identity_snapshot_written);

    let first = crate::session_persistence::load_at_root(&root)
        .unwrap()
        .expect("first Identity checkpoint");
    runtime
        .identity
        .transport()
        .cookie_jar()
        .add_cookie_str("usereg_session=fixture; Path=/; Secure", &self_service_url);
    runtime
        .persist_resume_state(&user())
        .expect("later account cookies do not replace the checkpoint");
    let after_self_service = crate::session_persistence::load_at_root(&root)
        .unwrap()
        .expect("Identity checkpoint remains present");
    assert_eq!(
        first.cookie_store().unwrap(),
        after_self_service.cookie_store().unwrap()
    );

    let restored = CampusCookieStore::default();
    restored
        .restore_bytes(&after_self_service.cookie_store().unwrap())
        .unwrap();
    assert!(restored.cookies(&identity_url).is_some());
    assert!(restored.cookies(&self_service_url).is_none());
...
#[tokio::test]
async fn backend_repair_usereg_device_reference_rejects_stale_snapshot_before_dispatch() {
    let identity = FixtureServer::new(vec![]);
    let mut runtime = runtime(&identity);
    let previous_generation = runtime.usereg_devices_generation;

    runtime.advance_usereg_device_generation();

    assert!(!runtime.usereg_device_reference_is_current(previous_generation, 0));
    assert_eq!(
        runtime
            .disconnect_usereg_device_for_sdk(previous_generation, 0)
            .await
            .unwrap_err(),
        "网络自助设备引用已过期，请刷新设备列表"
    );
...
#[test]
fn backend_repair_usereg_device_reference_is_invalidated_by_account_logout() {
    let identity = FixtureServer::new(vec![]);
    let mut runtime = runtime(&identity);
    runtime.usereg_devices_generation = 7;
    runtime
        .usereg_devices
        .push(crate::usereg_adapter::UseregDevice {
    ...
    assert!(runtime.usereg_device_reference_is_current(7, 0));

    runtime.logout().unwrap();

    assert_ne!(runtime.usereg_devices_generation, 7);
    assert!(!runtime.usereg_device_reference_is_current(7, 0));
    assert!(identity.requests().is_empty());
}
