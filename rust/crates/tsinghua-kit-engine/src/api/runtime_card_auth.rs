...
impl CampusRuntime {
    fn card_password_interaction_enabled(&self) -> bool {
    ...
    pub(super) fn has_terminal_card_password_boundary(&self) -> bool {
        self.terminal_interactive_auth && self.has_campus_card_password_challenge()
    }
    ...
    pub(crate) fn has_campus_card_password_challenge(&self) -> bool {
        self.card_password_interaction_enabled() && self.pending_card_password.is_some()
    }

    pub(crate) fn has_campus_card_second_factor_challenge(&self) -> bool {
        self.current_service_second_factor()
            .is_some_and(|pending| pending.target == ServiceSecondFactorTarget::CampusCard)
    }

    pub(crate) fn cancel_campus_card_password_challenge(&mut self) {
        self.pending_card_password = None;
    }

    pub(crate) async fn submit_campus_card_password(
        &mut self,
        password: Zeroizing<String>,
    ) -> Result<(), String> {
        self.complete_card_password(password).await
    }
    ...
    pub(super) async fn complete_terminal_card_password(
        ...
        if !self.terminal_interactive_auth {
            return Err("campus_card_password_boundary_invalid".to_owned());
        }
        self.complete_card_password(password).await
    }

    async fn complete_card_password(&mut self, password: Zeroizing<String>) -> Result<(), String> {
        ...
        if !self.card_password_interaction_enabled()
            || password.is_empty()
            || password.len() > 4096
    ...
    pub(super) async fn submit_card_target_password(
        ...
        let wire_password =
            Zeroizing::new(encrypt_password(&password, &public_key).map_err(|error| {
...
mod boundary_tests {
    ...
    async fn backend_repair_run072714_stale_or_foreign_card_boundary_never_submits() {
        for expired in [true, false] {
            ...
            let error = r
                .complete_card_password(Zeroizing::new("synthetic".to_owned()))
                .await
                .unwrap_err();
            assert_eq!(
    ...
    #[tokio::test]
    async fn backend_refactor_sdk_card_password_challenge_is_explicit_and_consumed_once() {
        let root = std::env::temp_dir().join(format!("sdk-card-boundary-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&root).unwrap();
        let mut runtime =
            super::super::create_sdk_runtime(&root.join("cache.json"), root.clone(), None).unwrap();
        let user = UserIdentity {
            username: "fixture-user".into(),
            display_name: None,
        };
        ...
        runtime
            ...
            .mark_authenticated(ServiceId::Identity, user.clone(), None, None, None)
            .unwrap();
        runtime.pending_card_password = Some(CardPasswordBoundary::new(
            user,
            "not-a-secret-public-key".into(),
            runtime.configured_card_origin().unwrap(),
        ));

        assert!(runtime.sdk_card_password_interaction);
        assert!(!runtime.terminal_interactive_auth);
        assert!(runtime.has_campus_card_password_challenge());
        let transport = runtime.identity.transport().clone();
        let fence = transport.replay_fence();
        let error = runtime
            .submit_campus_card_password(Zeroizing::new(String::new()))
            .await
            .unwrap_err();
        assert_eq!(
            crate::telemetry::diagnostic_reason(&error),
        ...
        assert!(!runtime.has_campus_card_password_challenge());
        assert!(fence.permits_probe_retry(&transport));
