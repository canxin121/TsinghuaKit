...
use crate::{
    api::thos::{
        ThosPhaseStepDto, ThosPhaseStepItemDto, ThosServiceDto, ThosServicesDto, ThosTaskDto,
        ThosTaskListDto,
    },
    read::{IncompleteReason, ReadCoverage},
    service_hall::{PendingTasks, Task as ServiceHallTask},
...
fn service_hall_task(task: ThosTaskDto) -> ServiceHallTask {
    ServiceHallTask::from_verified(
        task.id,
        task.title,
        task.status,
        task.node,
...
impl ThosClient {
    ...
    pub(crate) async fn pending(&self, counts: ThosCounts) -> Result<PendingTasks, ThosError> {
        ...
        let mut items: Vec<_> = todo
            .tasks
            .into_iter()
            .map(|task| task.dto)
            .map(service_hall_task)
            .collect();
        items.extend(
            active
                .tasks
        ...
        Ok(PendingTasks::from_verified(items, counts.todo, coverage))
    }
